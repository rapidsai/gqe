...
static_assert(std::constructible_from<any_device_resource, numa_device_accessible_resource>);
static_assert(!std::constructible_from<any_device_resource, numa_memory_resource>);
}  // namespace

template <bool DeviceAccessible>
basic_numa_memory_resource<DeviceAccessible>::basic_numa_memory_resource(page_kind::type page_kind)
  : _numa_node_set(), _page_kind(page_kind)
{
  _numa_node_set.add(0);
}

template <bool DeviceAccessible>
basic_numa_memory_resource<DeviceAccessible>::basic_numa_memory_resource(cpu_set numa_node_set,
                                                                         page_kind::type page_kind)
  : _numa_node_set(numa_node_set), _page_kind(page_kind)
{
}

template <bool DeviceAccessible>
void* basic_numa_memory_resource<DeviceAccessible>::allocate(cuda::stream_ref,
                                                             std::size_t bytes,
                                                             std::size_t alignment)
{
  ...
  if constexpr (DeviceAccessible) {
    // Register the allocation so GPU kernels can access it through CUDA host registration.
...
template <bool DeviceAccessible>
void basic_numa_memory_resource<DeviceAccessible>::deallocate(cuda::stream_ref,
                                                              void* ptr,
                                                              std::size_t bytes,
...
template class basic_numa_memory_resource<false>;
template class basic_numa_memory_resource<true>;
