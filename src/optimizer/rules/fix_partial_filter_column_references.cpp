...
class fix_partial_filter_column_references::apply_visitor : public gqe::logical::relation_visitor {
 public:
  apply_visitor(fix_partial_filter_column_references const& rule, bool& rule_applied)
  ...
  {
    // pre-order: fix this filter before recursing (matches original DOWN direction)
    const auto child = filter->children_unsafe()[0];
    if (child->type() == gqe::logical::relation::relation_type::read) {
      ...
      const auto base_table_columns  = _rule.get_catalog()->column_names(table_name);
      const auto projected_columns   = read_relation->column_names();
      auto original_filter_condition = filter->condition()->clone();
      auto updated_filter_condition =
        _rule.replace_column_references(base_table_columns, projected_columns, filter);
      _rule.set_expression(read_relation, std::move(updated_filter_condition));
      _rule.set_expression(filter, std::move(original_filter_condition));
      _rule_applied = true;
    }
    visit_children(filter);  // recurse after (pre-order)
  }

 private:
  fix_partial_filter_column_references const& _rule;
  bool& _rule_applied;
};

std::shared_ptr<gqe::logical::relation> fix_partial_filter_column_references::apply(
...
void fix_partial_filter_column_references::set_expression(
...
std::unique_ptr<gqe::expression> fix_partial_filter_column_references::replace_column_references(
