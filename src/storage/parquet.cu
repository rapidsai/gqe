...
[[nodiscard]] std::unique_ptr<cudf::column> cast(cudf::column_view input,
                                                 cudf::data_type type,
                                                 rmm::cuda_stream_view stream,
                                                 rmm::device_async_resource_ref mr)
{
  ...
  cudf::data_type dtype, std::vector<int64_t> keys, std::vector<cudf::size_type> num_rows)
{
  ...
  auto keys_dev = cudf::detail::make_device_uvector_async(
    keys, cudf::get_default_stream(), rmm::mr::get_current_device_resource_ref());

  auto offsets_dev = cudf::detail::make_device_uvector_async(
    offsets, cudf::get_default_stream(), rmm::mr::get_current_device_resource_ref());
