...
void* numa_pool_memory_resource::allocate(cuda::stream_ref stream,
                                          std::size_t bytes,
                                          std::size_t alignment)
{
  if (bytes == 0) { return nullptr; }
  return _pool_handle._impl->mr().allocate(stream, bytes, alignment);
}

void numa_pool_memory_resource::deallocate(cuda::stream_ref stream,
                                           void* ptr,
                                           std::size_t bytes,
                                           std::size_t alignment) noexcept
{
  if (ptr == nullptr) { return; }
  _pool_handle._impl->mr().deallocate(stream, ptr, bytes, alignment);
}

void* numa_pool_memory_resource::allocate_sync(std::size_t bytes, std::size_t alignment)
{
...
void numa_pool_memory_resource::deallocate_sync(void* ptr,
                                                std::size_t bytes,
                                                std::size_t alignment) noexcept
{
  auto const default_stream = cudf::get_default_stream();
