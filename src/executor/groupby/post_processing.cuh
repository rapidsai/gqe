...
namespace gqe {
struct groupby_simple_aggregations_collector {
...
inline std::tuple<cudf::table_view,
...
flatten_single_pass_aggs(cudf::host_span<cudf::groupby::aggregation_request const> requests)
{
  ...
  for (auto const& request : requests) {
    ...
    auto values_type = cudf::is_dictionary(request.values.type())
                         ...
                         : request.values.type();
    for (auto&& agg : agg_v) {
      auto simple_aggs = cudf::detail::aggregation_dispatcher(
        agg->kind, groupby_simple_aggregations_collector{}, values_type, *agg);
      for (auto& agg_s : simple_aggs) {
...
struct hash_compound_agg_finalizer {
  ...
  auto to_dense_agg_result(cudf::aggregation const& agg)
  {
    ...
    auto dense_result_table = gqe::groupby::detail::gather(cudf::table_view({std::move(s)}),
                                                           gather_map,
  ...
  auto gather_argminmax(cudf::aggregation const& agg)
  {
    ...
    auto gather_argminmax =
      gqe::groupby::detail::gather(cudf::table_view({col}),
                                   null_removed_map,
  ...
  template <cudf::aggregation::Kind k>
  void operator()(cudf::aggregation const& agg)
  {
...
inline std::vector<cudf::groupby::aggregation_result> extract_results(
  ...
  gqe::groupby::detail::result_cache& cache,
  rmm::cuda_stream_view stream,
  rmm::device_async_resource_ref mr)
{
  ...
  std::unordered_map<std::pair<cudf::column_view, std::reference_wrapper<cudf::aggregation const>>,
                     cudf::column_view,
                     gqe::groupby::detail::pair_column_aggregation_hash,
                     gqe::groupby::detail::pair_column_aggregation_equal_to>
    repeated_result;
