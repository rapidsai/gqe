...
void* pgas_memory_resource::allocate(cuda::stream_ref, std::size_t bytes, std::size_t alignment)
{
  std::size_t constexpr allocation_alignment = 256;
  if (alignment == 0 || alignment > allocation_alignment ||
      (allocation_alignment % alignment) != 0) {
    throw std::invalid_argument(
      "pgas_memory_resource supports alignments that evenly divide 256 bytes.");
  }
  if (_allocated) {
    GQE_LOG_ERROR(
...
void pgas_memory_resource::deallocate(cuda::stream_ref,
                                      void* ptr,
                                      std::size_t,
                                      std::size_t alignment) noexcept
{
  ...
  if (alignment == 0 || alignment > allocation_alignment ||
      (allocation_alignment % alignment) != 0) {
    GQE_LOG_ERROR(
      "Unsupported alignment {} in pgas_memory_resource::deallocate (supported divisors of 256).",
      alignment);
    return;
  }
