...
class StringToIntLiteralTest : public gqe::test::BaseFixtureWithParam<relation_t> {
 protected:
  ...
  // Register a table with `columns` in `cat`, build a read_relation against
  // it, and wrap that in a filter_relation whose condition is
  // `condition_expr` (with all columns projected).
  static std::shared_ptr<gqe::logical::relation> make_filter_plan(
    gqe::catalog& cat,
    std::string const& table_name,
    std::vector<gqe::column_traits> const& columns,
    std::unique_ptr<gqe::expression> condition_expr)
  {
    cat.register_table(table_name,
                       columns,
                       gqe::storage_kind::system_memory{},
                       gqe::partitioning_schema_kind::none{});
    ...
    for (auto const& c : columns) {
      ...
      column_types.push_back(c.data_type);
    }
    auto read_rel = std::make_shared<gqe::logical::read_relation>(
      subquery_relations, column_names, column_types, table_name, nullptr);

    std::vector<cudf::size_type> projection_indices;
    for (cudf::size_type i = 0; i < static_cast<cudf::size_type>(columns.size()); ++i) {
      projection_indices.push_back(i);
    }
    return std::make_shared<gqe::logical::filter_relation>(std::move(read_rel),
                                                           std::move(subquery_relations),
                                                           std::move(condition_expr),
                                                           std::move(projection_indices));
  }

  // Run the string_to_int_literal rule on `plan` against `cat`, then
  // EXPECT_EQ the resulting filter relation's condition against
  // `expected_condition`.
  static void expect_optimized_to(gqe::catalog& cat,
                                  std::shared_ptr<gqe::logical::relation> plan,
                                  gqe::expression const& expected_condition)
  {
    gqe::optimizer::optimization_configuration config(
      {gqe::optimizer::logical_optimization_rule_type::string_to_int_literal}, {});
    auto local_optimizer = std::make_unique<gqe::optimizer::logical_optimizer>(&config, &cat);
    auto optimized       = local_optimizer->optimize(plan);
    auto opt_filter      = dynamic_cast<gqe::logical::filter_relation const*>(optimized.get());
    EXPECT_EQ(expected_condition, *opt_filter->condition());
  }
...
// DataFusion's type coercion wraps an INT8 column in Cast(STRING, …) when
// it sees a comparison against a STRING literal. The rule should peel the
// cast and rewrite to the same INT8 == int8_t literal as the bare-column
// case. The cast itself is dropped from the output.
//
// Input:    Cast(STRING, col(0:Int8)) == "R"
// Expected: col(0:Int8) == int8(82)
TEST_F(StringToIntLiteralTest, peelsCastStringWrapperOnInt8Column)
{
  gqe::catalog cat{get_task_manager_ctx()};

  auto col   = std::make_shared<gqe::column_reference_expression>(0);
  auto cast  = std::make_shared<gqe::cast_expression>(col, cudf::data_type(cudf::type_id::STRING));
  auto lit   = std::make_shared<gqe::literal_expression<std::string>>("R");
  auto input = std::make_unique<gqe::equal_expression>(cast, lit);

  auto plan = make_filter_plan(
    cat, "t_peel", {{"a", cudf::data_type(cudf::type_id::INT8)}}, std::move(input));

  auto expected_col = std::make_shared<gqe::column_reference_expression>(0);
  auto expected_lit = std::make_shared<gqe::literal_expression<int8_t>>(82);  // 'R'
  auto expected     = gqe::equal_expression(expected_col, expected_lit);
  expect_optimized_to(cat, plan, expected);
}

// A cast whose target is something other than STRING should not be peeled —
// only the STRING-targeting cast (DataFusion's coercion shape) is recognized.
//
// Input:    Cast(INT16, col(0:Int8)) == "R"
// Expected: unchanged
TEST_F(StringToIntLiteralTest, doesNotPeelCastWithNonStringTarget)
{
  gqe::catalog cat{get_task_manager_ctx()};

  auto col   = std::make_shared<gqe::column_reference_expression>(0);
  auto cast  = std::make_shared<gqe::cast_expression>(col, cudf::data_type(cudf::type_id::INT16));
  auto lit   = std::make_shared<gqe::literal_expression<std::string>>("R");
  auto input = std::make_unique<gqe::equal_expression>(cast, lit);
  auto expected_input = input->clone();

  auto plan = make_filter_plan(
    cat, "t_nonstring_cast", {{"a", cudf::data_type(cudf::type_id::INT8)}}, std::move(input));
  expect_optimized_to(cat, plan, *expected_input);
}

// If the cast wraps something other than a column_reference (e.g. a binary op
// over two columns), the rule should not match. Peel only happens when the
// inner expression is a bare column_reference.
//
// Input:    Cast(STRING, add(col(0:Int8), col(1:Int8))) == "R"
// Expected: unchanged
TEST_F(StringToIntLiteralTest, doesNotPeelWhenCastInputIsNotColumnReference)
{
  gqe::catalog cat{get_task_manager_ctx()};

  auto col_a = std::make_shared<gqe::column_reference_expression>(0);
  auto col_b = std::make_shared<gqe::column_reference_expression>(1);
  auto add   = std::make_shared<gqe::add_expression>(col_a, col_b);
  auto cast  = std::make_shared<gqe::cast_expression>(add, cudf::data_type(cudf::type_id::STRING));
  auto lit   = std::make_shared<gqe::literal_expression<std::string>>("R");
  auto input = std::make_unique<gqe::equal_expression>(cast, lit);
  auto expected_input = input->clone();
  ...
  auto plan = make_filter_plan(
    cat,
    "t_compound",
    {{"a", cudf::data_type(cudf::type_id::INT8)}, {"b", cudf::data_type(cudf::type_id::INT8)}},
    std::move(input));
  expect_optimized_to(cat, plan, *expected_input);
}

// If the column wrapped by the cast is not INT8, the rule should not fire —
// the catalog's column type still matters after the cast peel.
//
// Input:    Cast(STRING, col(0:Int32)) == "5"
// Expected: unchanged
TEST_F(StringToIntLiteralTest, doesNotPeelWhenWrappedColumnIsNotInt8)
{
  gqe::catalog cat{get_task_manager_ctx()};

  auto col   = std::make_shared<gqe::column_reference_expression>(0);
  auto cast  = std::make_shared<gqe::cast_expression>(col, cudf::data_type(cudf::type_id::STRING));
  auto lit   = std::make_shared<gqe::literal_expression<std::string>>("5");
  auto input = std::make_unique<gqe::equal_expression>(cast, lit);
  auto expected_input = input->clone();

  auto plan = make_filter_plan(
    cat, "t_int32_col", {{"a", cudf::data_type(cudf::type_id::INT32)}}, std::move(input));
  expect_optimized_to(cat, plan, *expected_input);
}

// A multi-character STRING literal should not be rewritten — the rule's
// char-as-byte semantics only apply to single ASCII bytes. Same guard as the
// existing rule, exercised here through the cast-wrapped form.
//
// Input:    Cast(STRING, col(0:Int8)) == "RR"
// Expected: unchanged
TEST_F(StringToIntLiteralTest, doesNotRewriteMultiCharLiteralUnderCastWrapper)
{
  gqe::catalog cat{get_task_manager_ctx()};

  auto col   = std::make_shared<gqe::column_reference_expression>(0);
  auto cast  = std::make_shared<gqe::cast_expression>(col, cudf::data_type(cudf::type_id::STRING));
  auto lit   = std::make_shared<gqe::literal_expression<std::string>>("RR");
  auto input = std::make_unique<gqe::equal_expression>(cast, lit);
  auto expected_input = input->clone();

  auto plan = make_filter_plan(
    cat, "t_multichar", {{"a", cudf::data_type(cudf::type_id::INT8)}}, std::move(input));
  expect_optimized_to(cat, plan, *expected_input);
}
