  ...
  cudf::size_type* global_mark_counter_ref)
{
  ...
  using key_type_for_predicate = typename MarkSetRefType::key_type;
  auto predicate =
    cuco::detail::equal_wrapper<key_type_for_predicate, ComparatorAdapterType, allow_duplicates>{
  ...
  for (cudf::size_type i = grid.thread_rank() / cg_size; i < loop_bound;
    ...
    if (is_active) {
      ...
      auto probing_iter =
        probing_scheme.template make_iterator<MarkSetRefType::bucket_size>(query, bucket_extent);
      cuco::detail::equal_result status{};
      do {
        auto mutable_entry = storage.data() + *probing_iter;
    ...
    _mark_set{static_cast<size_t>(_build_num_valid_rows),
              ...
              cuco::empty_key{
                cuco::pair{detail::unset_mark(std::numeric_limits<cudf::hash_value_type>::max()),
                           cudf::detail::row::rhs_index_type{-1}}},  // empty_key_sentinel
              ...
              rmm::mr::polymorphic_allocator<key_type>{},
              stream.value()},
    ...
    _is_cached(is_cached)
{
  ...
  const create_input_pair_from_column<cudf::hash_value_type, cudf::detail::row::rhs_index_type>
  ...
  rmm::device_async_resource_ref mr) const
{
  ...
  using ProbeKeyType        = cuco::pair<cudf::hash_value_type, cudf::detail::row::lhs_index_type>;
  auto const d_probe_hasher = build_hasher_type{probe_equality_device_view};
  const create_input_pair<build_hasher_type, cudf::detail::row::lhs_index_type> probe_input_pair{
    d_probe_hasher};
