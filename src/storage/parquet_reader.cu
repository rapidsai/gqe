                                        ...
                                        rmm::device_async_resource_ref mr)
{
  ...
  for (auto const& file_path : file_paths) {
    ...
    // Validate the footer length against the actual file size before allocating, so a corrupted or
    // malicious file cannot trigger a huge (up to 4 GiB) allocation. The footer is preceded by the
    // file contents and followed by the 4B footer length and the 4B magic number, so the footer
    // length cannot exceed `file_size - 8`.
    struct stat file_stat;
    if (fstat(fd, &file_stat) == -1) {
      throw std::runtime_error(std::string("Cannot stat Parquet file: ") + strerror(errno));
    }
    if (file_stat.st_size < 8 || footer_length > static_cast<uint64_t>(file_stat.st_size) - 8) {
      throw unsupported_error("Customized Parquet reader: Invalid Parquet footer length");
    }
