...
/**
 * @brief Information about a peer rank, distributed via gRPC by the node manager.
 *
 * Each task manager receives a complete list of these from the node manager
 * via the `SetPeerInfo` RPC, enabling direct NVSHMEM data transfers and
 * gRPC task-migration calls between peers without MPI allgather.
 */
struct peer_info {
  int32_t rank;              ///< NVSHMEM PE rank of this peer.
  void* pgas_base_ptr;       ///< Base pointer of this peer's PGAS symmetric heap allocation.
  std::string grpc_address;  ///< Task-migration gRPC server address (ip:port) of this peer.
};

/**
 * @brief Apply gqe's default gRPC server channel arguments.
 *
 * @param[in,out] builder Server builder to configure.
 */
...
class task_migration_service final : public ::proto::TaskMigration::Service {
 public:
   ...
   * FetchTaskMetadata. Query ID is read from the task.
   *
  ...
  /**
   * @brief Unregister a single task after its stage completes.
   *
   * @param[in] t Task to unregister.
   */
  void unregister_task(task const& t);

  /**
   * @brief Unregister all tasks for a query. Called after query execution completes
   *        as a cleanup sweep (catches error paths and root tasks).
   *
   * @param[in] query_id Query to unregister.
   */
  void unregister_query(utility::uuid query_id);
 ...
 private:
  ...
  std::unordered_map<utility::uuid, std::unordered_map<std::string, task*>>
    _task_map;  // map of query_id -> task_id -> task
...
class nvshmem_task_migration_client : public task_migration_client {
 public:
   ...
   * @param[in] peers Pre-built peer list (rank, PGAS base ptr, gRPC address) from the node manager
   */
  nvshmem_task_migration_client(nvshmem_communicator* comm, std::span<peer_info const> peers);
