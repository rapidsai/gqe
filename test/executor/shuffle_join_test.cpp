...
class ShuffleJoinTest : public gqe::test::BaseFixture {
 protected:
   ...
   *
   * Models a stage where the left input arrives already hash-partitioned (e.g. from
   * an upstream shuffle) and the right input is partitioned at runtime. The left
   * pre-partitioning is produced by calling `cudf::hash_partition` directly on the
   * combined left input rather than hard-coding bucket assignments, so the test
   * stays valid regardless of cuDF's current row-hash implementation (cf.
   * rapidsai/cudf#20796, which shifted bucket assignments for single-integer keys).
   * Both sides go through the same hash function, so equal keys co-locate.
   */
  void execute_shuffle_join_one_side()
  {
    ...
    // Build the full left input (ID, Value), then hash-partition it with the same
    // function GQE's runtime partition_task uses on the right side. Each per-partition
    // slice is fed in as a separate already-partitioned input task.
    int64_column_wrapper left_id({1, 2, 3, 4, 5, 6});
    int64_column_wrapper left_value({100, 200, 300, 400, 500, 600});
    std::vector<std::unique_ptr<cudf::column>> left_columns;
    left_columns.push_back(left_id.release());
    ...
    auto [left_partitioned, left_partition_offsets] =
      cudf::hash_partition(left_table->view(), /*columns_to_hash=*/{0}, num_partitions);
    ...
    // Materialize one input task per left partition slice.
    std::vector<std::shared_ptr<gqe::task>> left_partition_inputs;
    left_partition_inputs.reserve(num_partitions);
    for (int32_t i = 0; i < num_partitions; ++i) {
      auto const start = left_partition_offsets[i];
      auto const end   = left_partition_offsets[i + 1];
      auto slice_view  = cudf::slice(left_partitioned->view(), {start, end}).front();
      left_partition_inputs.push_back(std::make_shared<gqe::test::executed_task>(
        ctx_ref, left_task_id, stage_id, std::make_unique<cudf::table>(slice_view)));
    }
