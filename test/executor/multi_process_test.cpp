...
/// Helper to generate a NVSHMEM UID and create a communicator for tests.
/// In single-process tests (nranks=1) this is sufficient. For multi-process
/// tests the test harness would need to distribute the UID across processes.
nvshmemx_uniqueid_t generate_uid()
{
...
TEST(MultiProcessExecutionTest, commInitTest)
{
  ...
  auto comm = std::make_unique<gqe::nvshmem_communicator>(0, 1, uid);
  comm->init();
...
TEST(MultiProcessExecutionTest, SingleGPUExecution)
{
  ...
  // Build a trivial peer list for the single-rank case.
  // The actual PGAS base ptr will be set after context creation.
  gqe::peer_info self_peer{.rank = 0, .pgas_base_ptr = nullptr, .grpc_address = ""};

  auto task_manager_ctx =
    gqe::multi_process_task_manager_context::default_init(0, 1, uid, std::span{&self_peer, 1});
...
TEST(MultiProcessExecutionTest, TableMigration)
{
  ...
  auto comm = std::make_unique<gqe::nvshmem_communicator>(0, 1, uid);
  comm->init();
  auto opms      = gqe::make_optimization_parameters();
  auto pool_size = opms.max_query_memory.value_or(rmm::percent_of_free_device_memory(90));
  ...
  // Build a single-rank peer list
  gqe::peer_info self_peer{.rank          = comm->rank(),
                           .pgas_base_ptr = upstream_mr->get_local_base_ptr(),
                           .grpc_address  = server.get_server_address()};
  auto migration_client =
    std::make_unique<gqe::nvshmem_task_migration_client>(comm.get(), std::span{&self_peer, 1});
  ...
  query_ctx.query_id = gqe::utility::uuid::generate();
