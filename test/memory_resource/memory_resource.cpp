...
class MemoryResourceTest<gqe::memory_resource::boost_shared_memory_resource>
 ...
 public:
  ...
  std::unique_ptr<gqe::memory_resource::boost_shared_memory_resource> mr;
};
...
TYPED_TEST(MemoryResourceTest, AllocateZero)
{
  ...
  void* ptr = nullptr;
  EXPECT_NO_THROW(ptr = this->mr->allocate(rmm::cuda_stream_default, allocation_size));
  EXPECT_NO_THROW(this->mr->deallocate(rmm::cuda_stream_default, ptr, allocation_size));
}
...
TEST(SharedNumaPoolResourceTest, ExportedPool)
{
  ...
  void* ptr = mr.allocate(rmm::cuda_stream_default, 1024);
  ASSERT_NE(nullptr, ptr);

  EXPECT_NO_THROW(mr.deallocate(rmm::cuda_stream_default, ptr, 1024));
...
TEST(SharedNumaPoolResourceTest, ImportedPool)
{
  ...
  void* owner_ptr = owner.allocate(rmm::cuda_stream_default, 1024);
  ...
  EXPECT_NO_THROW(owner.deallocate(rmm::cuda_stream_default, owner_ptr, 1024));
