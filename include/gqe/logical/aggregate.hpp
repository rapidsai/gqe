...
class aggregate_relation : public relation {
 ...
 public:
  ...
  /**
   * @brief Return whether the `aggregate_perfect_hash` optimizer rule determined that this
   * aggregate's group-by keys are perfect-hashable (unique in the input and fixed-width).
   */
  [[nodiscard]] bool is_perfect_hashable() const noexcept { return _is_perfect_hashable; }

  /**
   * @brief Mark this aggregate's group-by keys as perfect-hashable.
   *
   * Should only be called by the `aggregate_perfect_hash` optimizer rule after it has verified
   * that all group-by keys are unique in the input and have fixed-width types.
   */
  void set_perfect_hashable(bool perfect_hashable) { _is_perfect_hashable = perfect_hashable; }

 private:
  std::vector<std::unique_ptr<expression>> _keys;
  std::vector<std::pair<cudf::aggregation::Kind, std::unique_ptr<expression>>> _measures;
  bool _is_perfect_hashable = false;
};
