...
// A unique, self-cleaning temporary directory for filesystem tests. The PID keeps concurrent
// test processes apart and a per-instance counter keeps directories within a process distinct.
struct scoped_temp_dir {
  std::filesystem::path path;

  scoped_temp_dir()
    : path(std::filesystem::temp_directory_path() /
           ("gqe_helpers_test_" + std::to_string(::getpid()) + "_" + std::to_string(next_id())))
  {
...
void touch(std::filesystem::path const& p)
{
  std::ofstream ofs(p);
  ofs << 'x';
}
...
TEST(HelpersTest, GetParquetFilesNonexistentPathThrows)
{
  auto const missing = std::filesystem::temp_directory_path() /
                       ("gqe_helpers_test_missing_" + std::to_string(::getpid()));
  std::filesystem::remove_all(missing);  // ensure it does not exist
  EXPECT_THROW(std::ignore = utility::get_parquet_files(missing.string()), std::invalid_argument);
}

TEST(HelpersTest, GetParquetFilesFilePathThrows)
{
  scoped_temp_dir dir;
  auto const file = dir.path / "data.parquet";
  touch(file);
  // Passing the file itself (not its directory) is not a directory -> rejected.
  EXPECT_THROW(std::ignore = utility::get_parquet_files(file.string()), std::invalid_argument);
}

TEST(HelpersTest, GetParquetFilesEmptyDirectoryThrows)
{
  scoped_temp_dir dir;
  EXPECT_THROW(std::ignore = utility::get_parquet_files(dir.path.string()), std::invalid_argument);
}

TEST(HelpersTest, GetParquetFilesReturnsParquetFilesRecursively)
{
  scoped_temp_dir dir;
  touch(dir.path / "a.parquet");
  touch(dir.path / "ignore.txt");  // non-parquet, should be excluded
  std::filesystem::create_directories(dir.path / "sub");
  touch(dir.path / "sub" / "b.parquet");  // nested, should be found

  auto const files = utility::get_parquet_files(dir.path.string());

  auto contains = [&](std::string_view name) {
    return std::any_of(files.begin(), files.end(), [&](std::string const& f) {
      return f.find(name) != std::string::npos;
    });
  };

  ASSERT_EQ(files.size(), 2u);
  EXPECT_TRUE(contains("a.parquet"));
  EXPECT_TRUE(contains("b.parquet"));
}
