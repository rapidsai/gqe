...
void partition_task::execute()
{
  ...
  // As of cuDF 26.06 (rapidsai/cudf#20937), hash_partition returns
  // `num_partitions + 1` offsets where `offsets[num_partitions] == num_rows`,
  // matching the convention `partition_offset(...)` already exposes.
  GQE_EXPECTS(static_cast<int32_t>(partition_offsets.size()) == _num_partitions + 1,
              "cudf::hash_partition returned unexpected number of partition offsets");
