...
#include "../executor/groupby/aggregation_target_type.hpp"
...
[[nodiscard]] std::vector<cudf::data_type> aggregate_relation::data_types() const
{
  ...
  for (auto [aggregation_kind, value] : measures_unsafe()) {
    if (aggregation_kind == cudf::aggregation::MEAN) {
      ...
      cudf::data_type output_type = gqe::detail::compute_target_type(
        value->data_type(input_relation->data_types()), aggregation_kind);
      output_type = gqe::detail::compute_target_type(output_type,
                                                     get_second_aggregation_kind(aggregation_kind));
...
[[nodiscard]] std::string aggregate_relation::to_string() const
{
  ...
  // Perfect hashing
  agg_relation_string +=
    "\t\"perfect hash\" : " + std::string(_is_perfect_hashable ? "true" : "false") + ",\n";
  // Children
